...
<body>
    ...
    <div class="sidebar d-flex flex-column">
        ...
        <nav class="nav flex-column px-3">
            ...
            <a class="nav-link {{ $currentRoute == 'profile.edit' ? 'active' : 'text-dark' }} rounded mb-1"
                href="{{ route('profile.edit') }}">
                <i class="bi bi-person-circle me-2"></i> Profil & Akun
            </a>

            @if($role === 'spv')
            <a class="nav-link {{ $currentRoute == 'user.index' ? 'active' : 'text-dark' }} rounded mb-1"
                ...
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="date" name="date_from" id="date_from" class="form-control"
                            value="{{ $chartData['date_from'] ?? '' }}">
                    </div>

                    <div class="col-md-3">
                        <label for="date_to" class="form-label">Sampai Tanggal</label>
                        <input type="date" name="date_to" id="date_to" class="form-control"
                            value="{{ $chartData['date_to'] ?? '' }}">
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                @php
                                $priorityClass = ['Tinggi' => 'danger', 'Sedang' => 'warning', 'Rendah' =>
                                'secondary'][$report->priority] ?? 'primary';
                                $detailRoute = ($role === 'spv') ? route('reports.edit', $report->id) :
                                route('reports.show', $report->id);
                                @endphp
                                <tr>
                                    <td>K3-{{ str_pad($report->id, 4, '0', STR_PAD_LEFT) }}</td>
                                    <td>{{ $report->type }}</td>
                                    <td><span class="badge text-bg-{{ $priorityClass }}">{{ $report->priority }}</span>
                                    </td>
