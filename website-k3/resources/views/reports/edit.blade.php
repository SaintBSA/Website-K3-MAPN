...
<body>
    ...
    <div class="sidebar d-flex flex-column">
        ...
        <nav class="nav flex-column px-3">
            ...
            @if(Auth::user()->role === 'spv')
            <a class="nav-link {{ $currentRoute == 'master.settings' ? 'active' : 'text-dark' }} rounded mb-1"
    ...
    <div class="main-content">
        ...
        <p class="text-muted mb-4">Laporan Diajukan oleh {{ $report->reported_by }} pada
            {{ $report->created_at->format('d F Y') }}</p>
            ...
            <form action="{{ route('reports.update', $report->id) }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="tanggalLapor"
                            value="{{ $report->created_at->format('d M Y, H:i') }}" readonly disabled>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="waktuKejadian"
                            value="{{ $report->incident_datetime ? \Carbon\Carbon::parse($report->incident_datetime)->format('d M Y, H:i') : 'N/A' }}"
                            readonly disabled>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="lokasi" value="{{ $report->location }}" readonly
                            disabled>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="jenisInsiden" class="form-label fw-semibold">Jenis Insiden</label>
                        <input type="text" class="form-control" id="jenisInsiden" value="{{ $report->type }}" readonly
                            disabled>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="dampak" value="{{ $report->impact }}" readonly
                            disabled>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="pihakTerlibat" class="form-label fw-semibold">Pihak/Orang yang Terlibat</label>
                        <input type="text" class="form-control" id="pihakTerlibat"
                            value="{{ $report->involved_parties ?? 'Tidak ada data' }}" readonly disabled>
                    </div>
                </div>

                <div class="mb-3">
                    <label for="deskripsi" class="form-label fw-semibold">Keterangan/Deskripsi Insiden</label>
                    <textarea class="form-control" id="deskripsi" rows="4" readonly
                        disabled>{{ $report->description }}</textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control @error('spv_feedback') is-invalid @enderror" id="spv_feedback"
                        name="spv_feedback" rows="5"
                        placeholder="Tuliskan hasil investigasi singkat, tindak lanjut yang diperlukan, atau keputusan peninjauan di sini.">{{ old('spv_feedback', $report->spv_feedback) }}</textarea>
                    @error('spv_feedback') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="status" class="form-label fw-semibold text-success">Status Insiden <span
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="priority" class="form-label fw-semibold text-success">Prioritas Insiden <span
                                class="text-danger">*</span></label>
                        <select class="form-select" id="priority" name="priority" required>
                            @foreach($priorities as $priority)
                            <option value="{{ $priority->name }}" @if($report->priority == $priority->name) selected
                                @endif>
                                {{ $priority->name }}
                            </option>
                            @endforeach
        ...
        <div
            class="log-entry border-start border-3 border-{{ $log->new_status == 'Closed' ? 'secondary' : 'secondary' }} ps-3 mb-3 card card-k3 p-3">
            <p class="small mb-1">
